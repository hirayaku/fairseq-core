...
def fsdp_wrap_expert(args, layer, min_num_params=0):
    # hth NOTE: not sure about the calculation of num_experts here, but it's only for training
    ...
    pg_size = process_group.size() if process_group is not None else 1
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False, is_moe_layer=False):
        ...
        # hth NOTE: checkpointing is less relevant for inference
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
        ...
        # hth NOTE: fsdp_wrap seems irrelevant to inference overall
        if not is_moe_layer or getattr(args, "ddp_backend", None) != "fully_sharded":
            layer = fsdp_wrap(layer, min_num_params=min_params_to_wrap)
        else:
            layer = fsdp_wrap_expert(args, layer, min_num_params=min_params_to_wrap)
