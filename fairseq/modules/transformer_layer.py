...
class FeedForwardNetwork(nn.Module):
    ...
    def __init__(self, args, embed_dim, ffn_dim, dropout_module=None):
        ...
        self.moe_skip_init = getattr(args, "moe_skip_init", False)
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size):
        if self.moe_skip_init:
            return skip_init(nn.Linear, input_dim, output_dim)
        else:
...
def make_experts(args, embed_dim, expert_ffn_dim, dropout_module) -> nn.ModuleList:
    ...
    # hth NOTE: expert parallelism, on each gpu we have `local_moe_expert_count` experts
    ...
    # hth NOTE: duplicate expert parameters in an interleaving fashion
    # from rank 0 to N-1, we have expert 0, 1, ..., e-1, 0, 1, ..., e-1, 0, ...
    else:
